...
import { StyleSheet, TextInput, Text, ScrollView, Modal, ActivityIndicator, View } from 'react-native';
import StepBar from './StepBar';
import BUTTON_COLORS from '../../Constants/Utilities/index';
import { useDispatch, useSelector } from 'react-redux';
import { addPostSelector, addPostSendingStateSelector } from '../../redux/selectors';
    ...
    return (
        <>
            ...
            <ScrollView style={{ paddingHorizontal: 25, height: '100%', backgroundColor: 'white', paddingTop: 10 }}>
                ...
                <TextInput placeholder='Nhập tiêu đề bài đăng' style={styles.input} value={title} onChangeText={text => dispatch(AddPostSlice.actions.setTitle(text))}></TextInput>
                <Text style={{ fontSize: 17 }}>Liên hệ với</Text>
                <TextInput placeholder='Nhập họ và tên' style={styles.input} value={contactName} onChangeText={text => dispatch(AddPostSlice.actions.setContactName(text))}></TextInput>
                <Text style={{ fontSize: 17 }}>Số điện thoại</Text>
                <TextInput placeholder='Nhập số điện thoại' style={styles.input} value={phoneNumber} onChangeText={text => dispatch(AddPostSlice.actions.setContactPhone(text))} keyboardType="phone-pad"></TextInput>
                <Text style={{ fontSize: 17 }}>Mô tả chi tiết</Text>
                <TextInput style={{...styles.input, textAlignVertical: 'top'}} value={description} onChangeText={text => dispatch(AddPostSlice.actions.setDescription(text))} multiline = {true} numberOfLines = {6}></TextInput>
