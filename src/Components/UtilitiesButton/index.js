...
import { addPostSelector, postSelector } from "../../redux/selectors";
...
const Utilities = (props) => {
  ...
  const utilities = useSelector(
    addPost !== undefined && addPost ? addPostSelector : postSelector
  ).utilities;

  const changeIconColor = (name) => {
    if (iconClicked) {
      let newColor =
        utilities[name] === BUTTON_COLORS.colorBasic
          ? BUTTON_COLORS.colorPicked
          : BUTTON_COLORS.colorBasic;
      if (addPost) {
        dispatch(
          AddPostSlice.actions.utilitiesColorUpdate({
            ...utilities,
            [name]: newColor,
          })
        );
        ...
        dispatch(
          UpdatePostSlice.actions.utilitiesColorUpdate({
            ...utilities,
            [name]: newColor,
          })
        );
    ...
    return (
      <>
        <FontAwesome5.Button
          ...
          style={styles.buttonIcon}
        />
        <Text style={{ ...styles.textIcon, color: utilities[name] }}>
          WC riêng
        </Text>
      </>
    );
